...
Each user also gets their own unique identification token after registering. This token is used in every API call made, and is checked in the back-end every time before returning any data. This is, again, to prevent unauthorized access to user data. 

Finally, this app is linked to iO's AWS application hosting environment, which contains several security measures to prevent any unauthorized access, data leaks or other unwanted occurrences from happening.
